...
import utils.poi_within_poly as pwp
...
def encode_image_array_as_png_str(image):
  ...
  Args:
    image: a numpy array with shape [height, width, 3].F
...
#            display_str = '{}%'.format(int(100*0.99))
          else:
            display_str = '{}: {}%'.format(display_str, int(100*scores[i]))
#            display_str = '{}: {}%'.format(display_str, int(100*0.99))
...
##begin 自定义识别区域 
    ...
    ymin_ = ymin*im_width
    ymax_ = ymax*im_width
    xmin_ = xmin*im_height
    xmax_ = xmax*im_height
#    result = False
    pts = np.array([[xmin_,ymin_],[xmax_,ymin_],[xmax_,ymax_],[xmin_,ymax_]], np.int32)
    if not type(detect_area) == type(False):
        for point in pts:
            _result = pwp.isPoiWithinPoly(point,detect_area)
            if _result:
                result = True
                break
    else:
